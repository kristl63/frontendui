...
export const LessonAddRemoveGroupButton = ({ lesson, group }) => {
    ...
    const onChangeValue = (value) => {
        if (value) {
            ...
            dispatch(InsertLessonGroupAsyncAction({ lesson_id: lesson.id, group_id: group.id }))
                .then(onResolve, onReject);
        }
        else {
            const [onResolve, onReject] = validatorLessonAddRemoveGroupButtonB(dispatch);
            dispatch(DeleteLessonGroupAsyncAction({ lesson_id: lesson.id, group_id: group.id }))
                .then(onResolve, onReject);
        }
    };
